...
def _plan_success_mocks(tmp_path: Path) -> tuple[PlanOutput, str]:
    ...
    return plan_output, raw_plan_json
...
def test_run_plan_orchestration_parse_failure_prints_footer(tmp_path: Path) -> None:
    settings = _settings(tmp_path)
    _plan_success_mocks(tmp_path)
    raw_plan_json = json.dumps({"format_version": "1.2", "errored": False})
    ...
    with (
        ...
        plan_logic.run_plan(PlanInput(settings=settings, orchestration_active=True))

    footer_mock.assert_called_once()
